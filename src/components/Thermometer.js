...
export default class Thermometer extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      percent: () => this.props.value / this.state.max * 100,
      intervals: []
    }

    for (let step = 0; step <= this.state.steps; step++) {
      let val = ((this.state.max / this.state.steps) * step)
      let percent = (val / this.state.max) * 100
      let interval = { percent: percent, label: '$' + val.toLocaleString() }
      ...
      <div style={height} className={`thermometer ${ size } ${ theme }`}>
        ...
        <div className="thermometer__meter">
          ...
          <div style={heightPercent} className="thermometer__mercury">
            <div className="thermometer__percent-current">${value.toLocaleString()}</div>
